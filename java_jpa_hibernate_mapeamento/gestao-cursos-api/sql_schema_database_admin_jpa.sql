...
CREATE TABLE Curso (id  integer, nome varchar(255), sigla varchar(255), materialCurso_id bigint not null, professor_id bigint not null, primary key (id), unique (materialCurso_id));
CREATE TABLE Endereco (id  integer, bairro varchar(255), cep integer, cidade varchar(255), endereco varchar(255), estado varchar(255), logradouro varchar(255), numero varchar(255), endereco_id bigint, primary key (id));
